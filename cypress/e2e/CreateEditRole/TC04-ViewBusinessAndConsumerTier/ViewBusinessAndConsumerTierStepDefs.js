...
Then('user should view default badge with its type status', function(){
    permissionsManagementPage.tableList().each(function($el){
        if($el.find('.badge:visible').length > 0){
          cy.wrap($el).should('be.visible').then(function(){
            cy.wrap($el).find('td:nth-child(3)').invoke('text').then(function(val){
                if(val.includes('Business')){
                    expect(val.trim()).to.eq('Business')
                }
                else if(val.includes("Consumer")){
                    expect(val.trim()).to.eq("Consumer")
                }
            cy.wrap($el).find('td:nth-child(4)').invoke('text').then(function(val){
                expect(val.trim()).to.eq("Active")
            })
            })
            })
          }
